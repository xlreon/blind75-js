...
> Create a freq map of elements using a map, create bucket array where you add elements for same freq, iterate backwards in the bucket array and keep adding to result till its length is k.
...
> dfs throught the tree in post order, for each level find current max by adding left, right and node.val, check if that is max then update it and return node.val + max of left or right to parent and continue the process
